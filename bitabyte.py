    ...
    def paintEvent(self, event):
        ...
        for row in range(start_row, end_row):
            ...
            for col in range(cols):
                ...
                # Is this bit inside any highlighted pattern window?
                is_highlighted = any(
                    pos <= bit_index < pos + self.pattern_length
...
class TextDisplayWidget(QTextEdit):
    ...
    def display_binary(self):
        """Display bits as binary (0s and 1s) - OPTIMIZED"""
        html_parts = []
        html_parts.append('<pre style="font-family: Courier; font-size: 10pt; margin: 0; padding: 5px;">')

        for i in range(0, len(self.bits), self.width):
            ...
            for j, bit in enumerate(row_bits):
                bit_position = i + j
                is_highlighted = any(
                    pos <= bit_position < pos + self.pattern_length
                    for pos in self.highlighted_positions
                )
    ...
    def display_hex(self):
        """Display bits as hex (nibbles - 4 bits per character) - OPTIMIZED"""
        html_parts = []
        html_parts.append('<pre style="font-family: Courier; font-size: 10pt; margin: 0; padding: 5px;">')
            ...
            for nibble_idx in range(0, len(padded_bits), 4):
                ...
                for k in range(4):
                    ...
                    if bit_position < i + len(row_bits):
                        if any(pos <= bit_position < pos + self.pattern_length
                               for pos in self.highlighted_positions):
                            is_highlighted = True
                            break
...
class ByteStructuredTableWidget(QTableWidget):
    ...
    def set_frame_pattern(self, pattern_list):
        """
        Enable framing based on a sync pattern (list of ints bytes).
        ...
        data = self.bytes_data
        pattern = np.array(pattern_list, dtype=np.uint8)
        plen = len(pattern)
        if plen > len(data):
            self.frames = None
            self.frame_pattern = None
        ...
        for i in range(0, len(data) - plen + 1):
            if np.array_equal(data[i:i + plen], pattern):
                starts.append(i)
    ...
    def highlight_constant_columns(self):
        """
        Mark columns where the byte value is constant across all rows/frames.
            ...
            for col in range(max_len):
                ...
                for frame in frames:
                    ...
                    if val is None:
                        ...
                        constant = False
                        break
                if constant and val is not None:
                    self.constant_columns.add(col)
    ...
    def _format_undefined_bits(self, byte_slice, start_bit, num_bits):
        """
        Format undefined bits as binary (no prefix).
        """
    ...
    def _format_bit_range(self, row_bytes, col_def: ColumnDefinition):
        ...
        # col_def.start_bit is relative to the first byte of the row/frame,
        # but row_bytes starts at col_def.start_byte, so we subtract that.
...
class BitViewerWindow(QMainWindow):
    ...
    def on_mode_changed(self):
        ...
        if previous_mode == "bit" and new_mode == "byte":
            ...
            if self.operations:
                reply = QMessageBox.question(
                    self,
                    "Switch to Byte Mode",
                    "You have applied bit operations.\n\n"
                    "Do you want to save the processed bits as the new frame for Byte Mode?",
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
                )

                if reply == QMessageBox.StandardButton.Yes:
                    # Use processed bits, always zero-pad to full bytes
                    bits_to_save = self.bits.copy()
                    padding = (8 - (len(bits_to_save) % 8)) % 8
                    if padding:
                        bits_to_save = np.append(
                            bits_to_save,
                    ...
                    # Revert to original bytes if we have them
                    if self.original_bytes is not None:
                        self.bytes_data = self.original_bytes.copy()
                    elif self.original_bits is not None:
                        bits_to_save = self.original_bits.copy()
                        padding = (8 - (len(bits_to_save) % 8)) % 8
                        if padding:
                            bits_to_save = np.append(
                                bits_to_save,
                ...
                # No bit-ops applied: just zero-pad to full bytes
                bits_to_save = self.bits.copy()
                padding = (8 - (len(bits_to_save) % 8)) % 8
                if padding:
                    bits_to_save = np.append(
        ...
        # BYTE -> BIT
            ...
            if self.bytes_data is not None:
                # If framed, pad each frame to max length before unpacking,
                # and set bit-mode width to full frame width in bits.
    ...
    def frame_sync(self):
        ...
        text, ok = QInputDialog.getText(self, "Frame Sync Pattern", "Enter sync pattern (hex, e.g. 0xFFA2):")
        if not ok or not text.strip():
            return

        pattern = self.parse_hex_pattern(text)
        if pattern is None:
            QMessageBox.warning(self, "Invalid Pattern", "Could not parse hex pattern.")
            return

        self.save_state()
        frame_count = self.byte_table.set_frame_pattern(pattern)
        ...
        op_text = f"Frame Sync: {text.strip()}"
    ...
    def clear_frame_sync(self):
        ...
        self.operations.append(op_text)
        self.add_operation_to_list(op_text)
        self.status_label.setText("Frames cleared")
        ...
        while pos < len(self.bits):
            for op, num_str in matches:
                ...
                if op == 't':
                    ...
                    result.extend(self.bits[pos:end])
                    pos = end
                elif op == 'r':
                    end = min(pos + num, len(self.bits))
                    segment = self.bits[pos:end]
                    result.extend(segment[::-1])
                    pos = end
                elif op == 'i':
                    end = min(pos + num, len(self.bits))
                    segment = self.bits[pos:end]
                    result.extend(1 - segment)
                    pos = end
                elif op == 's':
                    pos = min(pos + num, len(self.bits))

        self.bits = np.array(result, dtype=np.uint8)
    ...
    def apply_delta_internal(self, window):
        if window == 1:
            ...
            result = []
            for i in range(window, len(self.bits), window):
                end_pos = min(i + window, len(self.bits))
                current_window = self.bits[i:end_pos]
                prev_start = i - window
                prev_end = prev_start + len(current_window)
                previous_window = self.bits[prev_start:prev_end]
                delta_window = np.bitwise_xor(current_window, previous_window)
                result.extend(delta_window)
    ...
    def apply_xor_internal(self, pattern):
        ...
        result = np.zeros_like(self.bits)
        pattern_len = len(pattern_bits)

        for i in range(len(self.bits)):
            pattern_bit = pattern_bits[i % pattern_len]
            result[i] = self.bits[i] ^ pattern_bit
    ...
    def open_file(self):
        ...
        if filename:
            try:
                ...
                self.bytes_data = byte_data.copy()

                self.original_bits = np.unpackbits(byte_data)
                self.bits = self.original_bits.copy()
                ...
                self.file_info_label.setText(
                    ...
                    f"Ones: {ones} ({ones / bit_count * 100:.1f}%)"
                )

                self.update_display()
                self.status_label.setText(f"Loaded {bit_count} bits / {file_size} bytes")
    ...
    def find_pattern_positions(self, pattern_bits):
        ...
        for i in range(len(self.bits) - pattern_len + 1):
            segment = self.bits[i:i + pattern_len]
            if max_errors == 0:
                if np.array_equal(segment, pattern_bits):
                    positions.append(i)
            else:
                differences = np.sum(segment != pattern_bits)
    ...
    def apply_takeskip(self):
        ...
        pattern, ok = QInputDialog.getText(
            self, "Take/Skip Pattern",
            "Enter pattern (e.g., t4r3i8s1):\nt=take, r=reverse, i=invert, s=skip"
        )
